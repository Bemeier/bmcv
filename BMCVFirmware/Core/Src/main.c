...
int main(void)
{
    ...
    while (1)
    {
        ...
        */
        ...
        {
            mcp_poll = 0;
            mcp_instance()->button_state[N_ENCODERS * 2 + 0] =
                HAL_GPIO_ReadPin(IN_BTN_MCU1_GPIO_Port, IN_BTN_MCU1_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 1] =
                HAL_GPIO_ReadPin(IN_BTN_MCU2_GPIO_Port, IN_BTN_MCU2_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 2] =
                HAL_GPIO_ReadPin(IN_BTN_MCU3_GPIO_Port, IN_BTN_MCU3_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 3] =
                HAL_GPIO_ReadPin(IN_BTN_MCU4_GPIO_Port, IN_BTN_MCU4_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 4] =
                HAL_GPIO_ReadPin(IN_BTN_MCU5_GPIO_Port, IN_BTN_MCU5_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 5] =
                HAL_GPIO_ReadPin(MENU_BTN_2_GPIO_Port, MENU_BTN_2_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 6] =
                HAL_GPIO_ReadPin(MENU_BTN_BOOT_GPIO_Port, MENU_BTN_BOOT_Pin);
            mcp_instance()->button_state[N_ENCODERS * 2 + 7] =
                HAL_GPIO_ReadPin(MENU_BTN_3_GPIO_Port, MENU_BTN_3_Pin);
        ...
        for (uint8_t ch = 0; ch < 4; ch++) {
          if (dacadc_instance()->trig_flag[ch]) {
            ...
            if (ch == 0) {
              ...
              trigger_envelope(&env[0], 10000 + slider * 20, 10000 + slider * 60, (int8_t) 127 -
        ((mcp_instance()->enc_position_state[7]*4) % 256));
            }
          }
        }
        */
...
static void MX_GPIO_Init(void)
{
    ...
    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOB, OUT_MCP_RESET_Pin | OUT_ADC_ADDR_Pin | LEVEL_SHIFTER_EN_Pin,
                      GPIO_PIN_RESET);
