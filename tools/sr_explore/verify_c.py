...
def parse_table(name, src):
    i = src.index(f"{name}[SR_MOD_BINS]")
...
def correction(vals, centre):
    """sr_norm_at(): the gain the pattern's own extremes ask for, floored, as an
    affine about slot 0."""
...
def main():
    src = open(HDR).read()
    C = parse_table("sr_centre_table", src)
    ...
    for s in SHAPES:
        for m in MODS:
            for li in range(len(M.SR_LENGTHS)):
                ...
                vals, _, _ = e.steps(s, m, M.SR_LENGTHS[li])
